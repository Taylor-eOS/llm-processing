...
def load_model_and_tokenizer():
    model = AutoModelForCausalLM.from_pretrained("nvidia/Nemotron-Research-Reasoning-Qwen-1.5B", torch_dtype=torch.bfloat16, device_map="auto")
...
def extract_after_think(text):
    ...
    if split_tag in text:
        return text.split(split_tag, 1)[1].lstrip()
    return text.strip()

def process_line(line, model, tokenizer, gen_kwargs, memory=None, use_memory=False):
    thinking_string = "" if settings.THINKING else " detailed thinking off."
    system_prompt = f"You are a text processing model. Output only the processed text itself, no other explanations or comments.{thinking_string}"
    user_prompt = ""
    if use_memory and memory:
        prev_original, prev_rewrite = memory
        user_prompt += f'Context: Previous input: "{prev_original}", Previous output: "{prev_rewrite}"\n'
    user_prompt += f"Task: {settings.REQUEST}\nInput sentence: \"{line}\"\nProcessed:"
    if settings.PRINT:
        print(system_prompt + "\n" + user_prompt)
    messages = [
    ...
    return extract_after_think(output_text)
